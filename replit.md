...
- **Data Sync Scheduler** (`server/scheduler.ts`): Runs a full sync cycle every 4 hours covering 8 data sources. Wave 1 (parallel): Rosters & IDs (AFL Fantasy API), Injuries & Lineups, Fixtures. Wave 2 (parallel): DFS Australia, Live Scores, Wheelo Ratings. Final (sequential): Footywire Prices (authoritative price source), then recalculate averages. Live scores poll every 2 minutes during active games. Per-source sync timestamps are tracked. Manual refresh available via POST `/api/data/refresh`. Sync status exposed via GET `/api/data/status` (public, no auth required). Frontend `DataStatusBar` component shows 8 sources with user-friendly names, sorted by priority. Displayed on the Players page.
...
- **AFL Fantasy API**: Syncs team, position, dual position on every startup. Also inserts new players (debutants). **Note**: As of 2026, the public API (`fantasy.afl.com.au/data/afl/players.json`) still serves 2025 season data (25 rounds of prices). Stale-season detection (maxRounds > currentRound+3) automatically skips price/stats updates from this source, using it only for roster/position/team changes. When the API is updated for 2026, price and stats syncing will resume automatically.
- **Footywire** (`footywire.com`): **Primary price authority.** Scrapes `dream_team_round` pages for rounds 1 through currentRound. The `CurrentSalary` column matches official AFL Fantasy prices exactly. Runs as the final step after DFS Australia, so Footywire prices override any earlier data. ~464 players matched across 3 rounds, 0 unmatched. Requires proper User-Agent header.
- **DFS Australia** (`dfsaustralia.com`): Downloads the AFL Fantasy 2026 XLSX spreadsheet on startup. Provides Champion Data IDs (= AFL Fantasy IDs), positions (including DPP), ownership %, PPM, prices, and historical stats for ~780 players. Primary source for linking players to AFL Fantasy IDs. Runs in scheduler wave 2. 890/895 players now linked.
