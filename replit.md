...
- **Wheelo Ratings** (`wheeloratings.com`): JSON API at `/src/match_stats/table_data/{roundId}.json`. Provides detailed per-round player match stats: AFL Fantasy scores (DreamTeamPoints), Supercoach scores, disposal efficiency, metres gained, clearances, CBA%, pressure acts, contested marks, intercept marks, ground ball gets, score involvements, rating points. Also provides team-level stats for team_context. Syncs all completed rounds on startup. Player matching by name+team. ~963 player rows across 3 rounds, 42 team context rows.
