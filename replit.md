...
- **Team Upload & Analyser**: Allows users to upload team screenshots for AI analysis and saving identified players. OCR uses GPT-4o vision with strict prompting to avoid hallucinated players. Fuzzy matching uses Levenshtein distance with tight thresholds (max 25% of name length). Screenshot prices are extracted by OCR and used to update player prices in the DB when saving, so team value matches the user's actual AFL Fantasy team. A "Clear Team" button (with confirmation dialog) allows users to wipe their entire team and re-upload fresh. "Load Glen's Team" loads a preset squad ($17.611M) matching the user's current AFL Fantasy Classic team.
