...
- **Data Sync Scheduler** (`server/scheduler.ts`): Runs a full sync cycle every 4 hours covering 7 data sources: Prices & Rosters (AFL Fantasy API), Live Scores (DFS Australia primary), Injuries & Lineups (combined AFL injury list + team lineups), Fixtures (Squiggle), Player Stats (Wheelo Ratings), Advanced Ratings (DFS Australia spreadsheet), and News & Intel (RSS feeds + club feeds). Live scores poll every 2 minutes during active games. Per-source sync timestamps are tracked. Manual refresh available via POST `/api/data/refresh`. Sync status exposed via GET `/api/data/status` (public, no auth required). Frontend `DataStatusBar` component shows 7 sources with user-friendly names, sorted by priority. Displayed on the Players page.
