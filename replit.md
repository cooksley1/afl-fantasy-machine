...
- **Live Scores**: `server/services/live-scores.ts` tracks live match statuses and fantasy scores, with round navigation (prev/next), manual bulk entry, per-player quick edit, and auto-fetch from Squiggle API (when available). `weekly_stats` table includes `goals_kicked`, `behinds_kicked`, `frees_against` columns. Match expansion shows ALL players from both teams (not just user's team), with user's players highlighted. `POST /api/live-scores/fetch-scores` attempts to pull individual player stats from Squiggle. Opening Round context message when round=0. `currentRound` defaults to 0 for Opening Round.
...
- **Player Data Management**: `server/expand-players.ts` loads and reconciles 780 real 2026 AFL players with seed data. Includes `repairPlayerData()` that runs on startup to fix missing byeRound, venue, startingPrice, breakEven, and clears fake recentScores only for rookies (gamesPlayed=0). Experienced players keep their 2025 baseline scores.
